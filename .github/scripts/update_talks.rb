...
talks_regex = /(?:Some recent talks I've given at conferences include:)([\s\S]*?)(?=\n\n)/m
updated_content = readme_content.match(talks_regex) ? readme_content.sub(talks_regex, talks_list.join("\n")) : readme_content.sub("<!-- recent_talks_placeholder -->", talks_list.join("\n"))
